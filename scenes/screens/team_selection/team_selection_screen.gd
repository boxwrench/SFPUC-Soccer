...
const FLAG_ANCHOR_POINT := Vector2(85, 65)
...
var flag_nodes: Array[TextureRect] = []
...
func try_navigate(selector_index: int, direction: Vector2i) -> void:
	...
	if rect.has_point(selection[selector_index] + direction):
		...
		selectors[selector_index].position = flag_nodes[flag_index].position
		SoundPlayer.play(SoundPlayer.Sound.UI_NAV)
...
func place_flags() -> void:
	for j in range(NB_ROWS):
		for i in range(NB_COLS):
			...
			flag_texture.position = FLAG_ANCHOR_POINT + Vector2(80 * i, 50 * j)
			var country_index := 1 + i + j * NB_COLS
			var country: String = DataLoader.get_countries()[country_index]
			flag_texture.texture = FlagHelper.get_selection_texture(country)
			flag_texture.scale = Vector2.ONE
			flag_texture.z_index = 1
			flags_container.add_child(flag_texture)
			flag_nodes.append(flag_texture)

			var team_label := Label.new()
			team_label.position = flag_texture.position + Vector2(-10, 30)
			team_label.custom_minimum_size = Vector2(64, 9)
			team_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
			team_label.add_theme_font_size_override("font_size", 5)
			team_label.text = TEAM_LABELS.get(country, country.to_upper())
			team_label.z_index = 1
			flags_container.add_child(team_label)
...
func add_selector(control_scheme: Player.ControlScheme) -> void:
	...
	selector.position = flag_nodes[0].position
